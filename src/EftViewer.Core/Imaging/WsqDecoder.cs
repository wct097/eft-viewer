// WSQ Decoder - Adapted from Managed.Wsq (https://github.com/grandchamp/Managed.Wsq)
// Original code from CodeProject WSQ implementation
// Modified to remove System.Drawing dependency for cross-platform compatibility

using System;
using System.Text;

namespace EftViewer.Core.Imaging
{
    /// <summary>
    /// Result of WSQ decoding containing raw grayscale pixel data.
    /// </summary>
    public class WsqDecodedImage
    {
        /// <summary>
        /// Raw grayscale pixel data (8-bit, row-major order).
        /// </summary>
    ...
    /// <summary>
    /// Decoder for WSQ (Wavelet Scalar Quantization) fingerprint images.
    /// </summary>
    public class WsqDecoder
    {
        /// <summary>
        /// Decode WSQ-compressed image data to raw grayscale pixels.
        /// </summary>
        /// <param name="data">WSQ compressed data</param>
        /// <returns>Decoded image with raw pixel data</returns>
        public WsqDecodedImage Decode(byte[] data)
        {
            ...
            // Flip the image vertically (WSQ stores bottom-to-top)
            var flippedData = new byte[cdata.Length];
            for (int y = 0; y < height; y++)
            {
                int srcRow = (height - 1 - y) * width;
                int dstRow = y * width;
                Array.Copy(cdata, srcRow, flippedData, dstRow, width);
            }

            return new WsqDecodedImage
            {
                Pixels = flippedData,
                Width = width,
                Height = height,
